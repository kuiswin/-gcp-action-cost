#!/usr/bin/env python3
"""
Step 3: Step1 (単価表) と Step2 (使用中サービス) を合成して .data/target_pricing.json に保存 (ハイブリッド)
"""
...
def main():
    print("================================================================================")
    print("【Step 3】 ハイブリッド単価マッピング (Step 1 × Step 2)")
    print("================================================================================")

    if not os.path.exists(CATALOG_FILE):
        print("❌ Error: 01_catalog_pricing.py を先に実行してください。", file=sys.stderr)
        sys.exit(1)
    if not os.path.exists(SERVICES_FILE):
        print("❌ Error: 02_active_services.py を先に実行してください。", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"・検出サービス数  : {len(active_names)} 件")

    if "Cloud Run" in active_names:
    ...
    if "Cloud Storage" in active_names:
    ...
    if any("Gemini" in s or "Vertex" in s for s in active_names):
